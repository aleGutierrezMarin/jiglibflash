package
{
	...
	public class RagdollTest extends BasicView
	{
		...
		private function initObjects():void
		{
			...
			ballBody.mass = 10;
			ballBody.moveTo(new Vector3D( -100, 30, -100));
		...
		private function resetRagdoll():void {
			...
			{
				ballBody.moveTo(new Vector3D(0, 800, 0));
			}
		}
		...
		protected override function onRenderTick(event:Event = null):void {
			...
			if(keyLeft)
			{
				ballBody.addWorldForce(new Vector3D(-100,0,0),ballBody.currentState.position);
			}
			if(keyRight)
			{
				ballBody.addWorldForce(new Vector3D(100,0,0),ballBody.currentState.position);
			...
			if(keyForward)
			{
				ballBody.addWorldForce(new Vector3D(0,0,100),ballBody.currentState.position);
			}
			if(keyReverse)
			{
				ballBody.addWorldForce(new Vector3D(0,0,-100),ballBody.currentState.position);
			...
			if(keyUp)
			{
				ballBody.addWorldForce(new Vector3D(0, 100, 0), ballBody.currentState.position);
			}
			resetRagdoll();
