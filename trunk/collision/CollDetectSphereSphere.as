...
package jiglib.collision {
	...
	public class CollDetectSphereSphere extends CollDetectFunctor {
		...
		{
			var sphere0:JSphere = info.body0.BodySkin as JSphere;
			var sphere1:JSphere = info.body1.BodySkin as JSphere;
			
			var delta:JNumber3D = JNumber3D.sub(sphere0.Position, sphere1.Position);
			...
			if (dist < radSum + JConfig.collToll)
			{
				...
				var worldPos:JNumber3D = JNumber3D.add(sphere1.Position, JNumber3D.multiply(delta, sphere1.Radius - 0.5 * depth));
				
				var collPts:Array = new Array();
				var cpInfo:CollPointInfo = new CollPointInfo();
				cpInfo.R0 = JNumber3D.sub(worldPos, sphere0.Position);
				cpInfo.R1 = JNumber3D.sub(worldPos, sphere1.Position);
